...
for i in range(0,loaded):
    ...
    current_state = '0'
    print(f"StartA: {current_state}")
    current_stated='0'
    print(f"StartB: {current_stated}")
    matrixoutA=[]
    matrixoutB=[]
    matrixoutC=[]
    matrix_out_list=[]
    ...
    # Choose the next state based on the current state's probabilities
        ...
        print("Steady-State VectorA:", res)
        magnitudeK=0
        ...
        next_stated = random.choices(states, weights=transition_matrix[current_stated])[0]
        hlkorp=int(next_stated)
        #print(hlkop)
        resk=calculate_steady_state(matrix_out_list[hlkorp])
        print("Steady-State VectorB:", resk)
        magnitudeKt=0
